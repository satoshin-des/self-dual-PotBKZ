...
{
    int consecutive_non_insertion_count = 0; // number of consecutive not inserting vector to lattice this->basis
    ...
    NTL::mat_ZZ inserted_vecs;               // vectors of lattice this->basis vector and inserted shortest vector
    ...
    for (int j, i, k = 0, h, l; consecutive_non_insertion_count < n - 1;)
    {
        ...
        if (this->ENUM(coeff_vec, mu.block(k - 1, k - 1, dim_of_local_block_lattice, dim_of_local_block_lattice), B.segment(k - 1, dim_of_local_block_lattice), s, dim_of_local_block_lattice, 0.99 * B.coeff(k - 1)))
        {
            consecutive_non_insertion_count = 0;

            shortest_vec = coeff_vec * this->basis.block(k - 1, 0, dim_of_local_block_lattice, m);
            ...
            for (j = 0; j < m; ++j)
            {
                for (i = 0; i < k - 1; ++i)
                {
                    inserted_vecs[i][j] = this->basis.coeff(i, j);
                }
                inserted_vecs[k - 1][j] = shortest_vec.coeff(j);
                for (i = k; i < h + 1; ++i)
                {
                    inserted_vecs[i][j] = this->basis.coeff(i - 1, j);
                }
            }
            ...
            for (i = 1; i <= h; ++i)
            {
                for (j = 0; j < m; ++j)
                {
                    this->basis.coeffRef(i - 1, j) = NTL::to_long(inserted_vecs[i][j]);
                }
            }
            this->GSO(B, mu, n, m);
        }
        else
        {
            ...
            for (i = 0; i < h; ++i)
            {
                for (j = 0; j < m; ++j)
                {
                    this->basis.coeffRef(i, j) = NTL::to_long(inserted_vecs[i][j]);
                }
            }
